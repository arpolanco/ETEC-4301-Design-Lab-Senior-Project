...
void setup() {
  ...
  Serial.begin(115200);
  right_prop.attach(9); //attatch the right motor to pin 3
...
the balance*/
kp=analogRead(1);
kp = map(kp, 0, 627, 0, 400);
kp /= 100.0f;

ki=analogRead(2);
ki = map(ki, 0, 627, 0, 100);
ki /= 100.0f;

kd=analogRead(3);
kd = map(kd, 0, 627, 0, 400);
kd /= 100.0f;

/*First calculate the error between the desired angle and 
*the real measured angle*/
error = getRoll() - desired_angle;
